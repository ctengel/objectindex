...
(see ``schema.sql``), with one deliberate change from earlier releases: the
SHA-256 ``checksum`` is stored as its lowercase hex string in ``varchar(64)``
rather than as ``bytea``. See ``migrations/001_checksum_bytea_to_hex.sql``.
"""
